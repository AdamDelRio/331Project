...
```
```{r}
average_year <- final_data |> group_by(year) |> summarize(mean_happy = mean(happiness_score, na.rm = TRUE),
                                                          mean_alc = mean(alcohol_consumption, na.rm = TRUE))
ggplot(data = average_year, mapping = aes(x = year, y = mean_happy)) +
  geom_line()
```

```{r}
ggplot(data = average_year, mapping = aes(x = year, y = mean_alc)) +
  geom_line()
```
